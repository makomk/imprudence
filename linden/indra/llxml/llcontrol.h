...
public:
	...
	// -- start of legacy hack to support old code --
	BOOL getWarning(const std::string& name)
	{
		return getBOOL("Warn"+name);
	}
	
	void setWarning(const std::string& name, BOOL val)
	{
		setBOOL("Warn"+name, val);
	}

	BOOL addWarning(const std::string& name)
	{
		return declareBOOL("Warn"+name, true, "Warn about "+name);
	}
	// --- end of legacy hack ---
